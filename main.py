...
def check_for_double_new_lines(read_path: pathlib.Path) -> None:
    with read_path.open() as file:
        try:
            ...
            if index:
                raise SyntaxError(f"не более одной новой строки между наборами")
        except ValueError:
            pass
...
def check_for_keys(p: dict, b: dict) -> None:
    for index in range(len(p)):
        if not list(p.keys())[index] == list(b.keys())[index]:
            raise SyntaxError("неправильный порядок ключей в наборе b")
    if not p.keys() == b.keys():
        raise ValueError("ключи между наборами p и b не равны")
    ...
    with read_path.open() as file:
        ...
        for index, line in enumerate(file):
            ...
            if validate(line):
                ...
                raise SyntaxError(f"неправильный синтаксис в строке {index}: {line}")
